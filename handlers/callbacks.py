...
@router.callback_query(
    lambda c: c.data == "get_ht_netschool" or c.data == "get_ht_date"
)
...
async def add_ht_student(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "📅 Выберите дату для добавления домашнего задания:",
        reply_markup=await calendar.start_calendar(),
    )
    await callback.answer()
...
async def notify(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text, reply_markup=keyboard_settings_notify(callback.from_user.id)
    )
    await callback.answer()
...
async def toggle_notify_admins(callback: CallbackQuery):
    ...
    add_settings(
        ...
        not current_setting["boolean_notify_admins"],
    )
...
async def toggle_notify_new_answers(callback: CallbackQuery):
    current_setting = get_settings(
        callback.from_user.id, ["boolean_notify_new_answers"]
    )
    add_settings(
        callback.from_user.id,
        "boolean_notify_new_answers",
        not current_setting["boolean_notify_new_answers"],
    )
    await callback.message.edit_reply_markup(
        reply_markup=keyboard_settings_notify(callback.from_user.id)
    )
...
async def toggle_notify_new_homework(callback: CallbackQuery):
    current_setting = get_settings(
        callback.from_user.id, ["boolean_notify_new_homework"]
    )
    add_settings(
        callback.from_user.id,
        "boolean_notify_new_homework",
        not current_setting["boolean_notify_new_homework"],
    )
    await callback.message.edit_reply_markup(
        reply_markup=keyboard_settings_notify(callback.from_user.id)
    )
